...
def obtain_array(name: str) -> Optional[List[str]]:
    ...
    with tempfile.NamedTemporaryFile(dir='/tmp') as output_file:
        ...
        extra_binds = ['--bind', output_file.name, output_file.name,
                       '--ro-bind', 'PKGBUILD', '/tmp/PKGBUILD', '--chdir', '/tmp']
        command_export_array = UNTRUSTED_PREFIX + extra_binds + [  # type: ignore
            '/bin/bash', '-c', "source PKGBUILD && {}".format(
...
def update_pkgrel(
    ...
    pkgbuild = re.sub(r'''(?<=^pkgrel=)['"]?([\d.]+)['"]?''',
                      replacer, pkgbuild, count=1, flags=re.MULTILINE)
...
def _try_aur_url(name: str) -> bytes:
    ...
    for url in urls:
        ...
        if response.status_code == 200:
            logger.debug(
                "downloaded aur tarball '%s' from url '%s'", name, url)
...
def _get_aur_packager(name: str) -> Tuple[Optional[str], str]:
    ...
    maintainer: Optional[str] = maintainer_cell.text_content().strip().split(None, 1)[
        0]
...
def aur_pre_build(
    ...
    if maintainers:
        ...
        if error:
            raise Exception(
                'unexpected AUR package maintainer / packager', who)
...
def download_official_pkgbuild(name: str) -> List[str]:
    ...
    if repo in ('core', 'extra'):
        ...
        gitrepo = 'community'
    pkgbase = [r['pkgbase']
               for r in info['results'] if r['repo'] != 'testing'][0]

    tarball_url = 'https://github.com/archlinux/svntogit-%s/archive/refs/heads/packages/%s.tar.gz' % (
        gitrepo, pkgbase)
    ...
    for pkg in pkgs:
        ...
        for line in pkginfo.splitlines():
            if provides_pattern.match(line):
                raise Exception(
                    f'{pkg} has an unversioned library "provides" entry: {line[11:]}')
