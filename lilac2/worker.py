...
def lilac_build(
    ...
    try:
        ...
        run_cmd(
            ["sh", "-c", "rm -f -- *.pkg.tar.xz *.pkg.tar.xz.sig *.pkg.tar.zst *.pkg.tar.zst.sig"])
        ...
        pkgs = [x for x in os.listdir() if x.endswith(
...
def call_build_cmd(
    ...
    if build_prefix == 'makepkg':
        ...
        extra_args = ['--share-net', '--bind', pwd,
                      f'/tmp/{basename}', '--chdir', f'/tmp/{basename}']
        cmd = UNTRUSTED_PREFIX + extra_args + \
            ['makepkg', '--holdver']  # type: ignore
    else:
