...
def packages_need_update(
    ...
    if care_pkgs:
        lilacinfos = {k: v for k, v in repo.lilacinfos.items()
                      if k in care_pkgs}
    else:
        lilacinfos = repo.lilacinfos
    newconfig, update_on_counts, update_on_errors = _gen_config_from_lilacinfos(
        lilacinfos)
    ...
    errors: DefaultDict[Optional[str],
    ...
    error_owners: DefaultDict[Maintainer,
                              List[Dict[str, Any]]] = defaultdict(list)
